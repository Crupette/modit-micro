...
void idt_createEntry(uint8_t i, void (*handler)(), uint8_t selector, uint8_t flags){
    idt_descriptor_t *entry = &(idt.entries[i]);
    entry->lower.offset_1 = (uintptr_t)handler & 0xFFFF;
    entry->lower.offset_2 = ((uintptr_t)handler >> 16) & 0xFFFF;
    entry->offset_3 = ((uintptr_t)handler >> 32) & 0xFFFFFFFF;

    entry->lower.selector = selector;
    entry->lower.type_attr = flags;
}
