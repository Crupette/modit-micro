...
/*  Wrapper for reading and writing to the MSR
 *  cpu_chk_msr:
 *      Checks if the CPU has support for MSR's
 *  cpu_read_msr:
 *      Reads from MSR [id] and puts results in [lo] [hi]
 *      id: MSR identifier
 *      lo: Low bytes of the read result
 *      hi: High bytes of the read result
 *  cpu_write_msr:
 *      Writes to MSR [id] using [lo], [hi]
 *      id: MSR Identifier
 *      lo: Low bytes to write
 *      hi: High bytes to write
 * */
